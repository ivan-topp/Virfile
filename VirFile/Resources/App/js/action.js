...
$(document).ready(function() {
	    	...
	    	function(data, status){
	    		if(status != "success"){console.log("Error al realizar la peticion.");}
	    			...
	    			data.forEach(function(val, indx){
	    				$('#ListUsers').append('<tr class="User"><td><input type="checkbox" id="'+val.ID_User+'" class="'+val.User_Name+'" name="numero[]"></td><td>'+val.ID_User+'</td><td>'+val.User_Name+'</td><td>'+val.Name+'</td><td>'+val.Mail+'</td><td>'+val.empresa+'</td><td>'+val.User_Level+'</td></tr>');
	    				console.log(val);
	    			});
	    		}
	    });
	...
	$("#btndelete1").click(function(event){
		$("input:checkbox:checked").each(function() {
			...
			var id = $(this).attr('id');
			var Datos = { Action : "delete", ID : parseInt($(this).attr('id')), Name : $(this).attr('class')};
			$.post("./Front.php",
				Datos,
		    	function(data, status){
		    		if(status != "success"){console.log("Error al realizar la peticion.");}
